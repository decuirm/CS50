...
*   CS50, pset3
*
*   Implements Scramble with CS50. -- additions: draw, scramble, lookup;
*
*   Usage: scramble [#]
*
...
int main(int argc, string argv[])
{
    ...
    while (true)
    {
        ...
        if (now >= end)
        {
            printf("\033[31m"); // red using ANSI code
            printf("Time:  %d\n\n", 0);
            printf("\033[39m"); // default using ANSI code
            break;
        }
...
bool lookup(string s)
{
    ...
    {    
        ...
        // compare s to dictionary word
        int x = strncmp(s,dictionary.words[mid].letters, strlen(s));
...
void scramble(void)
{
    ...
    // save copy grid to temporary array
    for (int row = 0; row < DIMENSION; row++)
        for (int col = 0; col < DIMENSION; col++)
    ...
    for (int row = 0; row < DIMENSION; row++)           
        for (int col = 0; col < DIMENSION; col++)
                grid[row][col] = tmp[DIMENSION-col-1][row];
}
